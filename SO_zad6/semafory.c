...
sem_t *stworz_semafor(const char *nazwa){
	sem_t *adres;
	if((adres = sem_open(nazwa, O_CREAT | O_EXCL, 0644, 1)) == SEM_FAILED){
...
void wartosc_semafor(sem_t *sem, int *sval){
	if(sem_getvalue(sem, sval) == -1){
		perror("Błąd odczytania wartości semafora");
		exit(EXIT_FAILURE);
	}
}
...
void opusc_semafor(sem_t *sem){					/* jeśli wartość 0 - wstrzymanie procesu aż do inkrementacji */
	if(sem_wait(sem) == -1){
